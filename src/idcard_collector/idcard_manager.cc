...
int IdCardManager::Init(boost::shared_ptr<IdCardConfig>& config)
{
	...
	BaseLogInfo("IdCardCollector init manager ...");

	int idcard_count = config->GetIdCardCount();
	BaseLogInfo("IdCardCollector init module count : %d", idcard_count);
	for (int i = 0; i < idcard_count; i++)
	{
		IdCardConfigItem* item = (IdCardConfigItem*)config->GetIdCard(i);
		if (item == nullptr)
		{
			BaseLogWarn("IdCardCollector manager manufactor[%s] module[%s] not exist",
				item->GetManufactorName(), item->GetModuleName());
			continue;
		}

		IdCardModule* module = new IdCardModule(item);
		BaseLogInfo("IdCardCollector init manufactor[%s] module[%s] start",
			item->GetManufactorName(), item->GetModuleName());
		result = module->Init();
		if (result != IDCARD_ERROR_SUCCESS)
		{
			BaseLogWarn("IdCardCollector manager init manufactor[%s] module[%s] failed : %d",
				item->GetManufactorName(), item->GetModuleName(), result);
			continue;
		}

		_idcard_modules.push_back(module);

		BaseLogInfo("IdCardCollector init manufactor[%s] module[%s] finish",
			item->GetManufactorName(), item->GetModuleName());
	}

	if (result == IDCARD_ERROR_SUCCESS)
	{
		BaseLogInfo("IdCardCollector init manager successful");
	}
...
int IdCardManager::ReadCard(bool read_photo, IdCardReadCallback* callback)
{
	int result = IDCARD_ERROR_SUCCESS;
	...
	for (auto it = _idcard_modules.begin(); it != _idcard_modules.end(); it++)
	{
		...
		if (module)
		{
			...
			if (result != IDCARD_ERROR_SUCCESS)
			{
				BaseLogWarn("IdCardCollector manufactor[%s] module[%s] open port failed, result = %d",
					module->GetConfigItem()->GetManufactorName(), module->GetConfigItem()->GetModuleName(), result);
				continue;
			}
			...
			if (result != IDCARD_ERROR_SUCCESS)
			{
				BaseLogWarn("IdCardCollector manufactor[%s] module[%s] read card failed, result = %d",
					module->GetConfigItem()->GetManufactorName(), module->GetConfigItem()->GetModuleName(), result);
				module->Close();
				continue;
			}
			...
			if (callback)
			{
				callback->OnReadCompleted(new IdCardData(people_name, people_sex, people_nation,
	...
	int bitmap_size)
{
	...
	{
		Json::Value root;
		root["people_name"] = people_name;
		root["people_sex"] = people_sex;
		root["people_nation"] = people_nation;
		root["people_birthday"] = people_birthday;
		root["people_address"] = people_address;
		root["people_id_code"] = people_id_code;
		root["sign_department"] = sign_department;
		root["valid_start_date"] = valid_start_date;
		root["valid_end_date"] = valid_end_date;
...
// curl http://58.51.141.218:9090/idcard/add -X POST -d "name=人员名称&sex=性别&national=民族&birthday=出生日期&idcard_add=身份证地址&idcard_no=身份证号&issuance=签发机关&start_date=身份证有效期开始时间&end_date=身份证有效期结束时间&idcard_img=身份证base64图片数据"
// curl http://58.51.141.218:9090/idcard/add -H "Content-Type:application/json" -X POST --data "{\"name\": \"\u5f20\u4e09\", \"sex\": \"\u7537\", \"national\": \"\u6c49\",\"birthday\": \"2018-10-30\",\"idcard_add\": \"\u6e56\u5317\u7701\u6b66\u6c49\u5e02\u6d2a\u5c71\u533a\u5149\u8c37\u5927\u9053\u0033\u0032\u0035\u53f7\", \"idcard_no\": \"42089719850625\", \"issuance\": \"\u6d2a\u5c71\u516c\u5b89\u5c40\", \"start_date\": \"1990-01-25\",\"end_date\": \"2020-01-25\"}"
/*
name '人员名称',
sex '性别'
national '民族',
	...
	int bitmap_size)
{
	...
	do
	{
		...
		BaseLogInfo("IdCardCollector output network parse json : %d", res);
		if (!res)
		{
		...
		BaseLogInfo("IdCardCollector output network parse json_status : %d, %d", json_status.isInt(), json_status.isString());
		...
		BaseLogInfo("IdCardCollector output network parse status : %d", status);
		if (status != 200)
		{
			result = status;
